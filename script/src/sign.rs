...
use bytes::Bytes;
use chain::{
    JoinSplit, OutPoint, ShieldedOutput, ShieldedSpend, Transaction, TransactionInput,
...
impl TransactionInputSigner {
    pub fn signature_hash(
        ...
        match sigversion {
            SignatureVersion::ForkId if sighashtype.fork_id => {
                self.signature_hash_fork_id(input_index, input_amount, script_pubkey, sighashtype)
            }
            SignatureVersion::Base | SignatureVersion::ForkId => {
                self.signature_hash_original(input_index, script_pubkey, sighashtype)
            }
            SignatureVersion::WitnessV0 => {
                self.signature_hash_witness0(input_index, input_amount, script_pubkey, sighashtype)
            }
        }
    }
    ...
    pub fn signed_input(
        ...
        let sighashtype = Sighash::from_u32(sigversion, sighash);
    ...
    pub fn signature_hash_original(
        ...
        if self.version >= 3 && self.overwintered {
            return self
                .signature_hash_overwintered(input_index, script_pubkey, sighash)
                .unwrap();
        }
    ...
    fn signature_hash_witness0(
        ...
        let tx = Transaction {
            ...
            lock_time: self.lock_time,
            join_splits: self.join_splits.clone(),
            shielded_spends: self.shielded_spends.clone(),
            shielded_outputs: self.shielded_outputs.clone(),
            zcash: self.zcash,
            binding_sig: H512::default(),
            join_split_pubkey: H256::default(),
            join_split_sig: H512::default(),
        ...
        let mut signer = bip143::SigHashCache::new(&tx);
        signer.signature_hash(input_index, script_pubkey, input_amount, sighash)
    }
    ...
    fn signature_hash_fork_id(
        ...
        self.signature_hash_witness0(input_index, input_amount, script_pubkey, sighash)
    }
    ...
    pub fn signature_hash_overwintered(
        ...
        sig_hash_stream.append(&u32::from(sighash));
...
mod tests {
    ...
    fn test_signature_hash_simple() {
        ...
        let expected_signature_hash =
            "5fda68729a6312e17e641e9a49fac2a4a6a680126610af573caab270d232f850".into();
        ...
        let hash = input_signer.signature_hash(
            ...
            Sighash::from_u32(SignatureVersion::Base, SighashBase::All as u32),
        );
    ...
    fn run_test_sighash(
        ...
        let hash = signer.signature_hash_original(input_index, &script, sighash);
    ...
    fn test_sapling_sig_hash() {
        ...
        let hash = signer.signature_hash(
            ...
            &Script::from("1976a914507173527b4c3318a2aecd793bf1cfed705950cf88ac"),
            SignatureVersion::Base,
            Sighash::from_u32(SignatureVersion::Base, 1),
        );
